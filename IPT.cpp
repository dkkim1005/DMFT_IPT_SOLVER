...
int main(int argc,char* argv[])
{
	...
	while(true)
	{
		g0_iw = inverse(iOmega_n - std::pow(D/2,2)*G_iw);
		...
		std::cout<<"|G_init - G_fin| : "<<distance<<std::endl<<std::flush;
		std::cout<<"filling : "<<1-n<<std::endl<<std::flush;
		if(distance < CONV)
		{
	...
	if(argc >= 3)
	{
		std::cout<<"we are using Pade approximation..."<<std::endl<<std::flush;
